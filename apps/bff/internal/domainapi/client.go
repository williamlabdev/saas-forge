...
// Refresh redeems a refresh token for a new token pair. Like Login, this is
// unauthenticated — the refresh token IS the credential, so there is no
// bearer to send.
func (c *Client) Refresh(ctx context.Context, refreshToken string) (map[string]any, error) {
	return c.do(ctx, http.MethodPost, "/api/v1/auth/refresh", "", map[string]any{"refresh_token": refreshToken}, http.StatusOK)
}

// Logout revokes a refresh token. Also unauthenticated for the same reason as
// Refresh: the token being revoked is the credential, not a bearer.
func (c *Client) Logout(ctx context.Context, refreshToken string) error {
	_, err := c.do(ctx, http.MethodPost, "/api/v1/auth/logout", "", map[string]any{"refresh_token": refreshToken}, http.StatusOK)
	return err
}
...
func (c *Client) ListNotifications(ctx context.Context, bearer string, limit int, unread bool) ([]map[string]any, error) {
...
// UnreadNotificationCount answers the caller's own unread count, scoped by
// the domain to the JWT's current tenant (tenant_id = current tenant OR
// tenant_id IS NULL) the same way ListNotifications is.
func (c *Client) UnreadNotificationCount(ctx context.Context, bearer string) (int, error) {
	raw, err := c.do(ctx, http.MethodGet, "/api/v1/notifications/unread-count", bearer, nil, http.StatusOK)
	if err != nil {
		return 0, err
	}
	n, _ := raw["count"].(float64)
	return int(n), nil
}

// MarkNotificationRead marks one of the caller's own notifications read. The
// domain answers NOT_FOUND for a notification that exists but is not the
// caller's — this client does not distinguish that from a nonexistent id,
// which is the point: no existence leak.
func (c *Client) MarkNotificationRead(ctx context.Context, bearer, id string) (map[string]any, error) {
	path := fmt.Sprintf("/api/v1/notifications/%s/read", url.PathEscape(id))
	return c.do(ctx, http.MethodPatch, path, bearer, nil, http.StatusOK)
}

// MarkAllNotificationsRead marks every unread notification in the caller's
// current tenant scope read, returning the count of rows touched.
func (c *Client) MarkAllNotificationsRead(ctx context.Context, bearer string) (int, error) {
	raw, err := c.do(ctx, http.MethodPost, "/api/v1/notifications/read-all", bearer, nil, http.StatusOK)
	if err != nil {
		return 0, err
	}
	n, _ := raw["count"].(float64)
