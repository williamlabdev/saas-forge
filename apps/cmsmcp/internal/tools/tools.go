...
// of ADR-013 step 6, the three write tools of step 7, and the two component
// reads ADR-020 §10 added so cms_describe's "component" and "dynamiczone"
// fields can be resolved to their sub-fields.
//
...
type componentArgs struct {
	Name string `json:"name" jsonschema:"the component's name, as it appears in a field's component attribute in cms_describe, in a dynamiczone field's components list, or as a zone item's __component"`
}
...
func (r *Registry) Install(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Describe the content types this credential may work with: every field's key, type, " +
			...
			"write it, and the filter operators it supports. A field of type \"dynamiczone\" also lists the " +
			"components it accepts, in its components attribute. Returns declarations, never content.",
	}, func(ctx context.Context, _ *mcp.CallToolRequest, args describeArgs) (*mcp.CallToolResult, any, error) {
		return r.describe(ctx, args)
	})

	mcp.AddTool(s, &mcp.Tool{
		Name: "cms_list_components",
		Description: "List the reusable components some content type this credential may work with embeds: " +
			"name, label and ordered sub-fields. A field of type \"component\" in cms_describe names one of these " +
			"in its component attribute; its value in an entry is an object (or an array of objects when multiple) " +
			"with exactly those sub-field keys. A field of type \"dynamiczone\" instead lists SEVERAL of them in " +
			"its components attribute; its value is always an array, and each element is an object carrying " +
			"\"__component\": \"<name>\" plus that component's sub-field keys. Returns declarations, never content.",
	}, func(ctx context.Context, _ *mcp.CallToolRequest, _ struct{}) (*mcp.CallToolResult, any, error) {
		return r.call(ctx, func(ctx context.Context) (json.RawMessage, error) {
			return r.up.ListComponents(ctx, r.token)
		})
	})

	mcp.AddTool(s, &mcp.Tool{
		Name: "cms_get_component",
		Description: "Fetch one reusable component by name: its label, ordered sub-fields (key, type, required, " +
			"enum values, constraints) and the content types that embed it.",
	}, func(ctx context.Context, _ *mcp.CallToolRequest, args componentArgs) (*mcp.CallToolResult, any, error) {
		return r.call(ctx, func(ctx context.Context) (json.RawMessage, error) {
			return r.up.GetComponent(ctx, r.token, args.Name)
		})
	})
