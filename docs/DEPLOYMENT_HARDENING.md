...
For the *how* — bringing the stack up with `deploy/docker-compose.prod.yml`
and Caddy, backups, restore drills, alerting — see
...
Every call through the wired `Authorizer` (`cmd/server/providers.go`) is wrapped
by `authz.DecoratedAuthorizer` (`internal/pkg/authz/decorator.go`) and logged
via `log/slog`. Control how much with `AUTHZ_DECISION_LOG`:

| Value | Behaviour |
|---|---|
| `off` | nothing is logged |
| `deny` (default) | denies and evaluation errors only |
| `all` | also logs allows, at `DEBUG`, rate-capped to `AllowSampleLimit` (100/s) per process — denies and errors are never dropped |

An unrecognised value refuses to start, the same as an unrecognised
`AUTHZ_MODE` (§1); `validateRuntime` checks both.

A deny line (JSON handler) looks like this:

```json
{"time":"2026-09-10T09:00:00Z","level":"WARN","msg":"authz decision","authz.mode":"opa","authz.policy_version":"9f2c...ab12","authz.policy_label":"2026-09-10","authz.action":"content:update","authz.resource_type":"content_entry","authz.resource_id":"entry-123","authz.tenant_id":"tenant-1","authz.user_id":"user-1","authz.roles":["viewer"],"authz.tenant_role":"viewer","authz.allowed":false,"authz.reason":"FORBIDDEN","authz.duration_ms":1,"authz.error":"","authz.request_id":"req-42"}
```

- **Grep for denies**: `authz.allowed":false` (or, with a text handler,
  `grep 'authz decision' | grep 'authz.allowed=false'`).
- **Grep by tenant/action**: combine with `authz.tenant_id":"<id>"` or
  `authz.action":"<action>"`.
- **`authz.policy_version`** is the sha256 hex of the embedded
  `authz.rego` source (`authz.PolicyVersion()`); `authz.policy_label` is the
  optional `# policy-version:` first-line comment in that file
  (`authz.PolicyLabel()`). Use the version to pin a deny to the exact policy
  revision that produced it — the label is a human-readable cross-check, not
  a substitute for the hash.
...
```text
                                Internet
                                   ...
                                   ▼
                          ┌───────────────┐
                          │  Caddy :443   │   TLS terminates here
                          │ (deploy/Caddy │   (deploy/docker-compose.prod.yml
                          │     file)     │    + deploy/Caddyfile).
                          └───┬───┬───┬───┘
              ┌───────────────┘   │   └───────────────┐
              ▼                   ▼                    ▼
     ┌────────────┐       ┌────────────┐       ┌────────────┐
     │ BFF :4000  │       │ Delivery   │       │ CMS MCP    │   Each edge
     │  GraphQL   │       │   :4100    │       │   :4200    │   authenticates
     └─────┬──────┘       └─────┬──────┘       └─────┬──────┘   its own callers
           │                    │                     │          and mints its
           └────────────────────┼─────────────────────┘          own
                                 │  private network + X-Gateway-Secret
                                 │  (minted by BFF/Delivery/CMS MCP)
                                 ▼
                        ┌──────────────────┐   APP_ENV=production,
...
Caddy also holds **two routes that go straight to the Domain API**,
bypassing every edge above — both are deliberate, narrow exceptions, not a
general "Caddy can reach `:8080`" hole:

- `API_DOMAIN/ready` → `app:8080/ready`. No `X-Gateway-Secret` is attached
  or needed — `/ready` (with `/health`) is the one path `GatewayGuard`
  (`internal/pkg/authn/gateway.go`) exempts by name, because a health/
  readiness probe cannot carry the secret. This exemption is what makes the
  direct route safe: nothing else on the Domain API is reachable through it.
- `CONSOLE_DOMAIN/content-api/*` → `app:8080/*` (prefix stripped). Caddy
  injects `X-Gateway-Secret` itself here — see `deploy/Caddyfile`'s
  `handle_path /content-api/*` block — because the console SPA is a browser
  bundle with nowhere to keep that secret; every other route to the Domain
