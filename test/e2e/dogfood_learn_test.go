...
// Dogfood: a real downstream course corpus against the real API.
//
...
//
//	LEARN_CONTENT=/path/to/courses.generated.json \
//	  go test ./test/e2e/ -run TestDogfood_Learn -v
//
// What this is for: work on the delivery path has to name something that is
// broken NOW, or a named real-content scenario it serves. Synthetic seeds cannot answer that — they are shaped by
...
func TestDogfood_LearnCorpus(t *testing.T) {
	...
	if path == "" {
		t.Skip("set LEARN_CONTENT to a courses.generated.json to run this")
	}
