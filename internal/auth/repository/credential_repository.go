...
type CredentialRepository interface {
	...
	// RevokeAllForUserInTenant revokes only the un-revoked refresh tokens a
	// user holds FOR ONE TENANT (2.2c: member removal). Deliberately
	// narrower than RevokeAllForUser — evicting someone from tenant A must
	// not also sign them out of every other tenant they belong to.
	// tenantSlug matches refresh_tokens.tenant_id (F2), which is nullable
	// and populated per-login by StoreRefreshToken; rows predating F2, or
	// issued with no active tenant, are untouched by this method (they were
	// never scoped to this tenant to begin with).
...
func (r *PostgresCredentialRepository) RevokeAllForUserInTenant(ctx context.Context, userID uuid.UUID, tenantSlug string) error {
	_, err := r.pool.Exec(ctx, `
