...
type sentDeadLetterNotice struct {
	tenantID string
	roles    []string
	exclude  uuid.UUID
	kind     string
...
type fakeDeadLetterNotifier struct {
	sent []sentDeadLetterNotice
	err  error
}

func (n *fakeDeadLetterNotifier) NotifyTenantRoles(_ context.Context, tenantID string, roles []string, exclude uuid.UUID, kind, title, body, link string) error {
	n.sent = append(n.sent, sentDeadLetterNotice{tenantID: tenantID, roles: roles, exclude: exclude, kind: kind, title: title, body: body, link: link})
	return n.err
}
...
// contentRow(t, tenant) is defined in webhook_test.go — same package, reused
// here for the dead-letter notification tests below.
...
// --- dead-letter notification (ADR-023, deliverable 4c) --------------------

func TestProcessBatch_DeadLetterContentEventNotifiesOwnerAdmin(t *testing.T) {
	row := contentRow(t, "tenant-a")
	repo := &fakeRepo{claim: []Row{row}, failDead: true}
	notifier := &fakeDeadLetterNotifier{}
	// No webhooks wired, so deliverContent fails every time — standing in for
	// "retries exhausted against a broken endpoint" without a real HTTP sender.
	w := NewWorker(repo, &fakeClient{}, 10, 1, time.Minute, metrics.NewRegistry()).WithNotifier(notifier)

	require.NoError(t, w.processBatch(context.Background()))

	require.Len(t, notifier.sent, 1, "a content event's dead-letter must notify exactly once")
	got := notifier.sent[0]
	assert.Equal(t, "tenant-a", got.tenantID)
	assert.ElementsMatch(t, []string{"owner", "admin"}, got.roles)
	assert.Equal(t, uuid.Nil, got.exclude, "no human actor to exclude for a system dead-letter")
	assert.Equal(t, notifdomain.KindWebhookDead, got.kind)
	assert.Contains(t, got.body, EventContentEntryPublished, "the event type is safe to name — unlike a content type's label")
}

func TestProcessBatch_NonContentEventDeadLetterDoesNotNotify(t *testing.T) {
	row := validRow(t, EventUserUpdated) // user.* dead-letters go to MCP's own monitoring, not tenant notices
	repo := &fakeRepo{claim: []Row{row}, failDead: true}
	notifier := &fakeDeadLetterNotifier{}
	client := &fakeClient{fn: func(mcp.UpsertRequest) error { return errors.New("mcp down") }}
	w := NewWorker(repo, client, 10, 1, time.Minute, metrics.NewRegistry()).WithNotifier(notifier)

	require.NoError(t, w.processBatch(context.Background()))

	assert.Equal(t, uint64(1), w.Registry().OutboxDead.Load(), "still dead-lettered")
	assert.Empty(t, notifier.sent, "a user.* dead-letter has no tenant webhook endpoint to fix, so it must not notify")
}

func TestProcessBatch_RetryingContentEventDoesNotNotify(t *testing.T) {
	row := contentRow(t, "tenant-a")
	repo := &fakeRepo{claim: []Row{row}, failDead: false} // retries remain — not yet dead
	notifier := &fakeDeadLetterNotifier{}
	w := NewWorker(repo, &fakeClient{}, 10, 5, time.Minute, metrics.NewRegistry()).WithNotifier(notifier)

	require.NoError(t, w.processBatch(context.Background()))

	assert.Empty(t, notifier.sent, "a retrying, not-yet-dead failure is normal at-least-once delivery in flight — must not notify")
}

func TestProcessBatch_DeadLetterWithoutNotifierStillWorks(t *testing.T) {
	row := contentRow(t, "tenant-a")
	repo := &fakeRepo{claim: []Row{row}, failDead: true}
	w := NewWorker(repo, &fakeClient{}, 10, 1, time.Minute, metrics.NewRegistry()) // no WithNotifier call

	require.NoError(t, w.processBatch(context.Background()), "a deployment with no notification plane wired must not error or panic")
	assert.Equal(t, uint64(1), w.Registry().OutboxDead.Load())
}

func TestProcessBatch_DeadLetterMarkFailedErrorSkipsNotify(t *testing.T) {
	row := contentRow(t, "tenant-a")
	repo := &fakeRepo{claim: []Row{row}, failDead: true, failErr: errors.New("db write failed")}
	notifier := &fakeDeadLetterNotifier{}
	w := NewWorker(repo, &fakeClient{}, 10, 1, time.Minute, metrics.NewRegistry()).WithNotifier(notifier)

	require.NoError(t, w.processBatch(context.Background()))

	assert.Empty(t, notifier.sent, "the dead state was not confirmed committed, so no notify — after commit, never before")
}
