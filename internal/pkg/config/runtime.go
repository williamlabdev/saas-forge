...
type Runtime struct {
	...
	AppEnv    string
	AuthzMode string
	// AuthzDecisionLog is the raw AUTHZ_DECISION_LOG value ("off"/"deny"/
	// "all", default "deny") — parsed and validated by
	// authz.ParseDecisionLogPolicy, not here: config stays free of a
	// dependency on internal/pkg/authz, the same way it already does not
	// enumerate AuthzMode's opa|rbac|allow (see cmd/server's validateRuntime,
	...
	// SchedulePollInterval is how often the content scheduler looks for due
	// publish/unpublish schedules (ADR-017).
	//
	// It is spelled as a DURATION while its outbox neighbours above are spelled
	// as integer seconds, and the difference is deliberate: the outbox polls in
	// single-digit seconds where "2" is unambiguous, and this one polls in tens
	// of seconds to minutes, where "300" reads as either five minutes or five
	// hours depending on how carefully you read the variable name. An operator
	// stretching this knob is doing it to cut scan load, and the unit belongs
	// next to the number they are choosing.
	SchedulePollInterval time.Duration
	// MediaTransformPollInterval is how often the media transform worker looks
	// for pending renditions (ADR-019). A duration, for SchedulePollInterval's
	// reason. Shorter default than the scheduler's: an editor is usually
	// looking at the admin UI waiting for the thumbnail.
	MediaTransformPollInterval time.Duration
	JWTSecret                  []byte
	...
	MediaUseSSL    bool
	// MediaPublicEndpoint/MediaPublicUseSSL configure a SECOND presigning
	// client for PresignPost and presigned-GET URLs only. Server-side calls
	// (Stat, Get, Put, Remove) keep using MediaEndpoint. This exists because
	// the browser, not the app, is the one that dereferences a signed URL —
	// in docker-compose MediaEndpoint is a compose-network hostname
	// (`minio:9000`) the host machine cannot resolve, so the presigning
	// client needs the host-reachable name instead. SigV4 signs the Host
	// header, so the URL cannot be rewritten after signing; it must be built
	// with the right host from the start. Empty MediaPublicEndpoint = same as
	// MediaEndpoint (today's behavior; production S3/R2 needs no change).
	MediaPublicEndpoint string
	MediaPublicUseSSL   bool
}
...
func LoadRuntimeFromEnv() Runtime {
	rt := Runtime{
		...
		AuthzDecisionLog:   getenv("AUTHZ_DECISION_LOG", "deny"),
	...
	// 30s: the promise this feature makes is "published around the minute you
	// asked for", and the scan it drives is the one query in the system that
	// crosses tenants. A malformed or non-positive value keeps the default
	// rather than stopping the scheduler — the same shape as every knob above,
	// and here the alternative is a typo in an env file silently turning off
	// scheduled releases.
	rt.SchedulePollInterval = 30 * time.Second
	if d, err := time.ParseDuration(getenv("SCHEDULE_POLL_INTERVAL", "30s")); err == nil && d > 0 {
		rt.SchedulePollInterval = d
	}
	rt.MediaTransformPollInterval = 15 * time.Second
	if d, err := time.ParseDuration(getenv("MEDIA_TRANSFORM_POLL_INTERVAL", "15s")); err == nil && d > 0 {
		rt.MediaTransformPollInterval = d
	}
	...
	rt.MediaPublicEndpoint = strings.TrimSpace(os.Getenv("MEDIA_S3_PUBLIC_ENDPOINT"))
	rt.MediaPublicUseSSL = rt.MediaUseSSL
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("MEDIA_S3_PUBLIC_USE_SSL"))); v != "" {
		rt.MediaPublicUseSSL = v == "true" || v == "1"
	}
	return rt
}
