...
func clearRuntimeEnv(t *testing.T) {
	...
	for _, k := range []string{
		"AUTHZ_MODE", "AUTHZ_DECISION_LOG", "MCP_BASE_URL", "OUTBOX_POLL_SECONDS", "OUTBOX_BATCH_SIZE",
		"OUTBOX_MAX_RETRIES", "SCHEDULE_POLL_INTERVAL", "JWT_ACCESS_TTL_MINUTES", "JWT_REFRESH_TTL_DAYS",
		...
		"MEDIA_S3_ENDPOINT", "MEDIA_S3_USE_SSL", "MEDIA_S3_PUBLIC_ENDPOINT", "MEDIA_S3_PUBLIC_USE_SSL",
	} {
		t.Setenv(k, "")
	}
}
...
func TestLoadRuntimeFromEnv_Defaults(t *testing.T) {
	...
	assert.Equal(t, "deny", rt.AuthzDecisionLog,
		"unset AUTHZ_DECISION_LOG defaults to deny-only, not off — see authz.ParseDecisionLogPolicy")
	...
	assert.Equal(t, 30*time.Second, rt.SchedulePollInterval,
		"an empty or malformed SCHEDULE_POLL_INTERVAL keeps the default rather than stopping scheduled releases")
...
func TestLoadRuntimeFromEnv_Overrides(t *testing.T) {
	...
	t.Setenv("AUTHZ_DECISION_LOG", "all")
	...
	assert.Equal(t, "all", rt.AuthzDecisionLog)
...
// MediaPublicEndpoint defaults to empty (same as MediaEndpoint downstream in
// objectstore.New) and MediaPublicUseSSL defaults to whatever MediaUseSSL
// resolved to — not a hardcoded false — so an operator who sets
// MEDIA_S3_USE_SSL=true without also setting MEDIA_S3_PUBLIC_USE_SSL gets a
// public client that still uses TLS.
func TestLoadRuntimeFromEnv_MediaPublicEndpointDefaults(t *testing.T) {
	clearRuntimeEnv(t)
	t.Setenv("MEDIA_S3_ENDPOINT", "minio:9000")
	t.Setenv("MEDIA_S3_USE_SSL", "true")

	rt := LoadRuntimeFromEnv()
	assert.Empty(t, rt.MediaPublicEndpoint)
	assert.True(t, rt.MediaPublicUseSSL)
}

func TestLoadRuntimeFromEnv_MediaPublicEndpointOverrides(t *testing.T) {
	clearRuntimeEnv(t)
	t.Setenv("MEDIA_S3_ENDPOINT", "minio:9000")
	t.Setenv("MEDIA_S3_USE_SSL", "true")
	t.Setenv("MEDIA_S3_PUBLIC_ENDPOINT", "localhost:9000")
	t.Setenv("MEDIA_S3_PUBLIC_USE_SSL", "false")

	rt := LoadRuntimeFromEnv()
	assert.Equal(t, "localhost:9000", rt.MediaPublicEndpoint)
	assert.False(t, rt.MediaPublicUseSSL)
}
