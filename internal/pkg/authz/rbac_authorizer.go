...
func (r *RBACAuthorizer) Allow(ctx context.Context, in Input) error {
	...
	if isTenantPlaneAction(in.Action) {
		...
		case ActionTenantInviteAccept:
			return nil
		case ActionTenantInviteList, ActionTenantInviteRevoke:
			// Same shape as invite:create: owner/admin of the ACTIVE tenant.
			if sub.TenantID == "" {
				return apperrors.ErrForbidden
			}
			if sub.TenantRole == "owner" || sub.TenantRole == "admin" {
			...
			return apperrors.ErrForbidden
		case ActionTenantMemberList:
			// The roster is readable by ANY member (2.2c) — owner, admin,
			// editor, and viewer alike. Only an active membership is required.
			if sub.TenantID == "" {
				return apperrors.ErrForbidden
			}
			switch sub.TenantRole {
			case "owner", "admin", "editor", "viewer":
				return nil
			default:
				return apperrors.ErrForbidden
			}
		case ActionTenantMemberUpdate, ActionTenantMemberRemove:
			// Coarse gate only: owner/admin of the ACTIVE tenant. The finer
			// rules — admin cannot touch the owner role, nobody may
			// self-operate, the last owner is protected — need the TARGET's
			// live role and are enforced in the tenant service, which also
			// re-checks the CALLER's live role (JWT role alone is not
			// trusted for durable mutations; see tenant_service.go).
			if sub.TenantID == "" {
				return apperrors.ErrForbidden
			}
			if sub.TenantRole == "owner" || sub.TenantRole == "admin" {
	...
	switch in.Action {
	case ActionNotificationRead, ActionNotificationCreate, ActionNotificationMarkRead:
...
func isTenantPlaneAction(action string) bool {
	return strings.HasPrefix(action, "content:") ||
		...
		strings.HasPrefix(action, "tenant:member:") ||
...
func allowContentByTenantRole(tenantRole, action string) error {
	...
	// the switch below.
	//
	// The widening fired a trigger the ADR wrote down in advance:
	// notifyProposalFiled used to notify only for AGENT proposals, on the
	// reasoning that a human proposer was by construction an approver — an
	// editor is not, so an editor's proposal reached nobody until ADR-023
	// (2026-09-09) rewrote the notify to enumerate the tenant's owner/admin
	// members and exclude only the proposer. See proposal_notify.go.
