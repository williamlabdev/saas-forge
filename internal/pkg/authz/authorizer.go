...
const (
	...
	// ActionNotificationMarkRead covers both PATCH .../read (one notification)
	// and POST .../read-all (the caller's whole mailbox). One action, not two:
	// both verbs are "the caller may change the read state of their own
	// mailbox", and read-all's tenant scoping is enforced by the repository
	// query (WHERE user_id = caller), the same place PATCH's ownership check
	// lives — there is no second capability here for a second action name to
	// gate.
	ActionNotificationMarkRead = "notification:mark_read"
	...
	// ActionTenantInviteList covers reading a tenant's pending invites (2.2c).
	// owner/admin only — same roles as invite:create, since seeing who has been
	// invited (and at what role) is the same power as minting the invite.
	ActionTenantInviteList = "tenant:invite:list"
	// ActionTenantInviteRevoke covers cancelling a pending invite before it is
	// accepted (2.2c). owner/admin only. Separate verb from invite:create for
	// the reason agent:credential:revoke is separate from :issue above — "may
	// stop a pending grant, may not create one" is an ordinary thing to want.
	ActionTenantInviteRevoke = "tenant:invite:revoke"
	// ActionTenantMemberList covers reading a tenant's membership roster (2.2c).
	// Unlike invite management, this is readable by ALL FOUR roles — a viewer
	// legitimately wants to know who else is in the tenant, and the roster
	// carries no more sensitivity than the org chart it mirrors.
	ActionTenantMemberList = "tenant:member:list"
	// ActionTenantMemberUpdate covers changing an existing member's role,
	// including transferring/granting ownership (2.2c). owner/admin only at
	// this coarse gate; the finer rule that only an OWNER may assign or touch
	// the owner role, and that nobody may change their own role, is enforced
	// in the service layer where the target's live role is known (this verb
	// cannot see who the target is, only that the tenant is being touched).
	ActionTenantMemberUpdate = "tenant:member:update"
	// ActionTenantMemberRemove covers evicting a member from the tenant
	// (2.2c). owner/admin only at this coarse gate; admin-cannot-remove-owner,
	// last-owner protection, and no-self-removal are service-layer rules for
	// the same reason as ActionTenantMemberUpdate above.
