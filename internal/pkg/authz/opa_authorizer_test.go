...
func TestOPAAuthorizer_TenantPlane(t *testing.T) {
	...
	// Invite list/revoke (2.2c) in OPA mode mirror the facade: owner/admin only.
	require.NoError(t, auth.Allow(owner, Input{
		Action: ActionTenantInviteList, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))
	require.Error(t, auth.Allow(editor, Input{
		Action: ActionTenantInviteList, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))
	require.NoError(t, auth.Allow(owner, Input{
		Action: ActionTenantInviteRevoke, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))
	require.Error(t, auth.Allow(viewer, Input{
		Action: ActionTenantInviteRevoke, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))

	// Member roster (2.2c) in OPA mode: readable by any membership, mutated by
	// owner/admin only.
	require.NoError(t, auth.Allow(viewer, Input{
		Action: ActionTenantMemberList, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))
	require.NoError(t, auth.Allow(owner, Input{
		Action: ActionTenantMemberUpdate, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))
	require.Error(t, auth.Allow(editor, Input{
		Action: ActionTenantMemberUpdate, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))
	require.NoError(t, auth.Allow(owner, Input{
		Action: ActionTenantMemberRemove, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))
	require.Error(t, auth.Allow(viewer, Input{
		Action: ActionTenantMemberRemove, Resource: Resource{Type: "tenant", ID: "t_a"},
	}))
