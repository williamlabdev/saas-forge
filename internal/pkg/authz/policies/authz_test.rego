...
# --- tenant invites: list/revoke (2.2c) ------------------------------------------

test_admin_lists_invites if {
	allow with input as {
		"subject": {"user_id": "u1", "roles": [], "tenant_role": "admin"},
		"action": "tenant:invite:list",
		"resource": {"type": "tenant", "id": "t_a"},
		"context": {"tenant_id": "t_a"},
	}
}

test_editor_cannot_list_invites if {
	not allow with input as {
		"subject": {"user_id": "u1", "roles": [], "tenant_role": "editor"},
		"action": "tenant:invite:list",
		"resource": {"type": "tenant", "id": "t_a"},
...
test_viewer_cannot_revoke_invite if {
	not allow with input as {
		"subject": {"user_id": "u1", "roles": [], "tenant_role": "viewer"},
		"action": "tenant:invite:revoke",
		"resource": {"type": "tenant", "id": "t_a"},
...
test_no_membership_cannot_list_members if {
	not allow with input as {
		"subject": {"user_id": "u1", "roles": [], "tenant_role": ""},
		"action": "tenant:member:list",
		"resource": {"type": "tenant", "id": "t_a"},
...
test_editor_cannot_update_member if {
	not allow with input as {
		"subject": {"user_id": "u1", "roles": [], "tenant_role": "editor"},
		"action": "tenant:member:update",
		"resource": {"type": "tenant", "id": "t_a"},
...
test_viewer_cannot_remove_member if {
	not allow with input as {
		"subject": {"user_id": "u1", "roles": [], "tenant_role": "viewer"},
		"action": "tenant:member:remove",
		"resource": {"type": "tenant", "id": "t_a"},
...
test_platform_admin_no_member_blanket if {
	not allow with input as {
		"subject": {"user_id": "u1", "roles": ["admin"], "tenant_role": ""},
		"action": "tenant:member:list",
		"resource": {"type": "tenant", "id": "t_a"},
...
# --- notifications (ADR-023) -----------------------------------------------------
# The rule is the same shape as user:read/user:delete above: subject.user_id ==
# resource.id, no role needed — a mailbox is not tenant-plane administration.
# All three verbs share one rule, so one self/non-self pair speaks for all three
# rather than tripling the same two cases.

test_self_reads_own_notifications if {
	allow with input as {
		"subject": {"user_id": "11111111-1111-1111-1111-111111111111", "roles": []},
		"action": "notification:read",
		"resource": {"type": "notification", "id": "11111111-1111-1111-1111-111111111111"},
		"context": {},
	}
}

test_self_creates_own_notification if {
	allow with input as {
		"subject": {"user_id": "11111111-1111-1111-1111-111111111111", "roles": []},
...
test_self_marks_own_notification_read if {
	allow with input as {
		"subject": {"user_id": "11111111-1111-1111-1111-111111111111", "roles": []},
...
# Not self. Deliberately NOT an "admin" roles claim here — is_admin DOES reach
# this rule (notification: matches no tenant_plane_action prefix, so the
# general admin blanket applies, same as user:read above it), so a "does admin
# bypass self-scoping" case belongs under is_admin, not under this one. This
# case isolates the resource.id match on an otherwise-unprivileged subject.
test_non_self_cannot_read_notifications if {
	not allow with input as {
		"subject": {"user_id": "22222222-2222-2222-2222-222222222222", "roles": []},
		"action": "notification:read",
		"resource": {"type": "notification", "id": "11111111-1111-1111-1111-111111111111"},
		"context": {},
	}
...
test_non_self_cannot_create_notification if {
	not allow with input as {
		"subject": {"user_id": "22222222-2222-2222-2222-222222222222", "roles": []},
		"action": "notification:create",
		"resource": {"type": "notification", "id": "11111111-1111-1111-1111-111111111111"},
...
test_non_self_cannot_mark_notification_read if {
	not allow with input as {
		"subject": {"user_id": "22222222-2222-2222-2222-222222222222", "roles": []},
		"action": "notification:mark_read",
		"resource": {"type": "notification", "id": "11111111-1111-1111-1111-111111111111"},
...
# Agent-token deny is deliberately NOT a case here, for the same reason
# test_editor_publishes gives above content:publish: this rule only ever sees
# subject.user_id and resource.id, and the notification service constructs
# resource.id FROM the caller's own subject.UserID (authorizeSelf) — so an
# agent credential minted from this same user would still satisfy user_id ==
# resource.id at this layer. The refusal is real but lives one layer up, in
# Go's refuseUnlistedAgentAction (internal/pkg/authz/agent_gate.go), which
# runs BEFORE either policy is consulted and is enforced identically for the
# RBAC and OPA authorizers — see agent_gate_test.go's
# TestAgentCredentialIsRefusedUnlistedActions, whose agentActionExpectations
# table lists all three notification:* verbs as false. A green suite here is
# not evidence that gate works, exactly as noted above content:publish.
