...
tenant_plane_action if startswith(input.action, "tenant:member:")
...
allow if {
	input.action in {"notification:read", "notification:create", "notification:mark_read"}
	input.subject.user_id == input.resource.id
}
...
# List: owner/admin of the ACTIVE tenant only — same power as create.
allow if {
	input.action == "tenant:invite:list"
	input.context.tenant_id != ""
	input.subject.tenant_role in {"owner", "admin"}
}

# Revoke: owner/admin of the ACTIVE tenant only. Separate rule from create/list
# for the reason agent:credential:revoke is separate from :issue — "may stop a
# pending grant, may not create one" is worth being able to say on its own.
allow if {
	input.action == "tenant:invite:revoke"
	input.context.tenant_id != ""
	input.subject.tenant_role in {"owner", "admin"}
}

# --- tenant members (2.2c) ----------------------------------------------------
# List: readable by ANY member — owner/admin/editor/viewer alike. The roster
# carries no more sensitivity than an org chart; only viewers of CONTENT are
# read-restricted, and membership is not content.
allow if {
	input.action == "tenant:member:list"
	input.context.tenant_id != ""
	input.subject.tenant_role in {"owner", "admin", "editor", "viewer"}
}

# Update/remove: coarse gate is owner/admin of the ACTIVE tenant. The finer
# rules — admin cannot touch the owner role, nobody may self-operate, the last
# owner is protected — need the TARGET's live role, which this policy input
# does not carry; they are enforced in the Go tenant service, which also
# re-checks the CALLER's live role rather than trusting tenant_role alone for
# a durable mutation (same posture as tenant:invite:create).
allow if {
	input.action == "tenant:member:update"
	input.context.tenant_id != ""
	input.subject.tenant_role in {"owner", "admin"}
}

allow if {
	input.action == "tenant:member:remove"
	input.context.tenant_id != ""
	input.subject.tenant_role in {"owner", "admin"}
}
