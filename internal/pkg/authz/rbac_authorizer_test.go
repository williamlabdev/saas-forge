...
// 2.2c: listing/revoking invites is owner/admin only, same shape as create.
func TestRBACAuthorizer_InviteListAndRevoke(t *testing.T) {
	allow := func(role, activeTenant, action string) error {
		ctx := authn.WithSubject(context.Background(), authn.Subject{
			UserID: uuid.New(), TenantID: activeTenant, TenantRole: role,
		})
		return NewRBACAuthorizer().Allow(ctx, Input{
			Action: action, Resource: Resource{Type: "tenant", ID: activeTenant},
		})
	}
	for _, action := range []string{ActionTenantInviteList, ActionTenantInviteRevoke} {
		require.NoError(t, allow("owner", "t_x", action), action)
		require.NoError(t, allow("admin", "t_x", action), action)
		assert.ErrorIs(t, allow("editor", "t_x", action), apperrors.ErrForbidden, action)
		assert.ErrorIs(t, allow("viewer", "t_x", action), apperrors.ErrForbidden, action)
		assert.ErrorIs(t, allow("owner", "", action), apperrors.ErrForbidden, action)
	}
}

// 2.2c: the member roster is readable by every role that holds a membership;
// changing a role or removing a member is owner/admin only at this coarse
// gate (the finer per-target rules live in the tenant service).
func TestRBACAuthorizer_MemberActions(t *testing.T) {
	allow := func(role, activeTenant, action string) error {
		ctx := authn.WithSubject(context.Background(), authn.Subject{
			UserID: uuid.New(), TenantID: activeTenant, TenantRole: role,
		})
	...
	require.NoError(t, allow("owner", "t_x", ActionTenantMemberList))
	require.NoError(t, allow("admin", "t_x", ActionTenantMemberList))
	require.NoError(t, allow("editor", "t_x", ActionTenantMemberList))
	require.NoError(t, allow("viewer", "t_x", ActionTenantMemberList))
	assert.ErrorIs(t, allow("", "t_x", ActionTenantMemberList), apperrors.ErrForbidden)
	assert.ErrorIs(t, allow("owner", "", ActionTenantMemberList), apperrors.ErrForbidden)

	for _, action := range []string{ActionTenantMemberUpdate, ActionTenantMemberRemove} {
		require.NoError(t, allow("owner", "t_x", action), action)
		require.NoError(t, allow("admin", "t_x", action), action)
		assert.ErrorIs(t, allow("editor", "t_x", action), apperrors.ErrForbidden, action)
		assert.ErrorIs(t, allow("viewer", "t_x", action), apperrors.ErrForbidden, action)
...
func TestRBACAuthorizer_UnknownTenantPlaneVerbFailsClosed(t *testing.T) {
	...
	// which is why it was swapped rather than deleted. tenant:invite:revoke
	// stood here until 2.2c (ADR-022) made IT real too, and was swapped for
	// tenant:invite:resend, which nothing implements. If either of these is
	// ever implemented, swap it too; do not drop the case.
	for _, action := range []string{"content:archive", "tenant:invite:resend"} {
		err := NewRBACAuthorizer().Allow(platformAdmin, Input{
