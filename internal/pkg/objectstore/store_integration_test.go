...
func TestPresignRoundTrip(t *testing.T) {
	...
	// Get/Put are the worker's byte path (ADR-019): Put a derived object, Get
	// it back with the recorded content type, and a missing key is ErrNotFound
	// rather than a lazy error on first read.
	derived := []byte("derived bytes")
	if err := store.Put(ctx, key+".thumb.png", bytes.NewReader(derived), int64(len(derived)), "image/png"); err != nil {
		t.Fatalf("put: %v", err)
	}
	rc, info, err := store.Get(ctx, key+".thumb.png")
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	gotDerived, _ := io.ReadAll(rc)
	_ = rc.Close()
	if !bytes.Equal(gotDerived, derived) || info.Size != int64(len(derived)) || info.ContentType != "image/png" {
		t.Fatalf("get returned %q %+v", gotDerived, info)
	}
	if _, _, err := store.Get(ctx, key+".missing"); !errors.Is(err, objectstore.ErrNotFound) {
		t.Fatalf("get missing: err=%v want ErrNotFound", err)
	}
	if err := store.Delete(ctx, key+".thumb.png"); err != nil {
		t.Fatal(err)
	}
