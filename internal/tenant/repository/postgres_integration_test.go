...
func loadMigrationSQL() (string, error) {
	...
	for _, name := range []string{
		...
		"000015_plans.up.sql",
		"000048_tenant_invites_revoke_email.up.sql",
	} {
...
// --- member management / invite listing & revocation (2.2c, ADR-022) --------
//
// These exercise the REPOSITORY layer only: last-owner protection (a race
// guard) and the raw CRUD/state-machine shape. Authorization concerns (who
// may call this at all, self-operation refusal, admin-cannot-touch-owner)
// are service-layer questions and are covered by
// internal/tenant/service's unit tests instead — the repository trusts its
// caller on those and enforces only what a race could otherwise defeat.

// addMember inserts a membership row directly (bypassing AcceptInvite),
// used to build multi-member fixtures for the roster/role/removal tests.
func addMember(t *testing.T, tenantID, userID uuid.UUID, role string) {
	t.Helper()
...
func TestIntegration_Members_OrderedRoster(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	owner := seedUser(t)
	slug := provision(t, owner)
	tenant, err := repo.TenantBySlug(ctx, slug)
	if err != nil {
		t.Fatalf("tenant: %v", err)
	}
	editor := seedUser(t)
	addMember(t, tenant.ID, editor, domain.RoleEditor)
	if _, err := testPool.Exec(ctx,
		`UPDATE memberships SET created_at = created_at - INTERVAL '1 second' WHERE user_id = $1`, editor); err != nil {
		t.Fatalf("backdate: %v", err)
	}
	viewer := seedUser(t)
	addMember(t, tenant.ID, viewer, domain.RoleViewer)

	members, err := repo.Members(ctx, tenant.ID)
	if err != nil {
	...
	// Earliest membership first: editor was backdated ahead of both owner
	// (provisioned first, but not backdated) and viewer.
	if members[0].UserID != editor {
		t.Fatalf("order[0] = %s, want editor %s", members[0].UserID, editor)
	}
}

func TestIntegration_UpdateMemberRole_Succeeds(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	owner := seedUser(t)
	slug := provision(t, owner)
	tenant, _ := repo.TenantBySlug(ctx, slug)
	editor := seedUser(t)
	addMember(t, tenant.ID, editor, domain.RoleEditor)

	m, err := repo.UpdateMemberRole(ctx, tenant.ID, editor, domain.RoleViewer)
	if err != nil {
		t.Fatalf("update role: %v", err)
	}
	if m.Role != domain.RoleViewer || m.UserID != editor {
		t.Fatalf("got %+v", m)
	}
	role, err := repo.MembershipRole(ctx, editor, slug)
	if err != nil || role != domain.RoleViewer {
		t.Fatalf("role after update = %q err=%v", role, err)
	}
}

func TestIntegration_UpdateMemberRole_NotFound(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	if _, err := repo.UpdateMemberRole(ctx, tenant.ID, stranger, domain.RoleViewer); err == nil {
		t.Fatal("want error for non-member target")
	} else if ae, ok := apperrors.As(err); !ok || ae.Code != apperrors.ErrNotFound.Code {
		t.Fatalf("want NOT_FOUND, got %v", err)
	}
}

func TestIntegration_UpdateMemberRole_LastOwnerGuard(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	// Sole owner: demoting is refused.
	if _, err := repo.UpdateMemberRole(ctx, tenant.ID, owner, domain.RoleAdmin); err != domain.ErrLastOwner {
		t.Fatalf("demote sole owner err=%v, want ErrLastOwner", err)
	}

	// Add a second owner: now demoting the first succeeds.
	second := seedUser(t)
	addMember(t, tenant.ID, second, domain.RoleOwner)
	if _, err := repo.UpdateMemberRole(ctx, tenant.ID, owner, domain.RoleAdmin); err != nil {
		t.Fatalf("demote with a second owner present: %v", err)
	}

	// Now second is the sole owner: demoting IT is refused again.
	if _, err := repo.UpdateMemberRole(ctx, tenant.ID, second, domain.RoleAdmin); err != domain.ErrLastOwner {
		t.Fatalf("demote new sole owner err=%v, want ErrLastOwner", err)
	}
}

func TestIntegration_RemoveMember_Succeeds(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	if err := repo.RemoveMember(ctx, tenant.ID, editor); err != nil {
		t.Fatalf("remove: %v", err)
	}
	if _, err := repo.MembershipRole(ctx, editor, slug); err == nil {
		t.Fatal("removed member should have no membership left")
	}
}

func TestIntegration_RemoveMember_NotFound(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	if err := repo.RemoveMember(ctx, tenant.ID, stranger); err == nil {
		t.Fatal("want error for non-member target")
	} else if ae, ok := apperrors.As(err); !ok || ae.Code != apperrors.ErrNotFound.Code {
		t.Fatalf("want NOT_FOUND, got %v", err)
	}
...
func TestIntegration_RemoveMember_LastOwnerGuard(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	if err := repo.RemoveMember(ctx, tenant.ID, owner); err != domain.ErrLastOwner {
		t.Fatalf("remove sole owner err=%v, want ErrLastOwner", err)
	}

	second := seedUser(t)
	addMember(t, tenant.ID, second, domain.RoleOwner)
	if err := repo.RemoveMember(ctx, tenant.ID, owner); err != nil {
		t.Fatalf("remove with a second owner present: %v", err)
	}
	if err := repo.RemoveMember(ctx, tenant.ID, second); err != domain.ErrLastOwner {
		t.Fatalf("remove new sole owner err=%v, want ErrLastOwner", err)
	}
}

// TestIntegration_UpdateMemberRole_ConcurrentOwnersDemoteEachOther pins the
// one HTTP-reachable path to domain.ErrLastOwner (ADR-022 §3): two owners
// racing to demote EACH OTHER, which the service layer does not otherwise
// serialize (an owner acting on another owner isn't "an admin touching the
// owner role"). Before the fixed owner-lock ordering in lockOwnersOrdered
// (ADR-022 §3b), forcing this interleaving reproduced SQLSTATE 40P01
// (deadlock_detected) 5/5 times; this test asserts the three invariants
// that must hold instead: exactly one call succeeds, the other gets
// domain.ErrLastOwner (never a raw/opaque error), and the tenant still has
// exactly one owner afterward. Looped 20x to catch flakiness a single run
// could miss.
func TestIntegration_UpdateMemberRole_ConcurrentOwnersDemoteEachOther(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	for i := 0; i < 20; i++ {
		ownerA := seedUser(t)
		slug := provision(t, ownerA)
		tenant, err := repo.TenantBySlug(ctx, slug)
		if err != nil {
			t.Fatalf("iter %d: tenant: %v", i, err)
		}
		ownerB := seedUser(t)
		addMember(t, tenant.ID, ownerB, domain.RoleOwner)
		...
		go func() {
			defer wg.Done()
			_, errs[0] = repo.UpdateMemberRole(ctx, tenant.ID, ownerB, domain.RoleAdmin)
		}()
		go func() {
			defer wg.Done()
			_, errs[1] = repo.UpdateMemberRole(ctx, tenant.ID, ownerA, domain.RoleAdmin)
		}()
		wg.Wait()

		successes, lastOwnerRefusals, other := 0, 0, 0
		for _, err := range errs {
			...
			default:
				other++
				t.Errorf("iter %d: unexpected error (want nil or ErrLastOwner, e.g. no raw SQLSTATE 40P01): %v", i, err)
			}
		}
		if successes != 1 || lastOwnerRefusals != 1 || other != 0 {
			t.Fatalf("iter %d: got successes=%d lastOwnerRefusals=%d other=%d, want exactly 1/1/0",
				i, successes, lastOwnerRefusals, other)
		}

		var ownerCount int
		if err := testPool.QueryRow(ctx,
			`SELECT COUNT(*) FROM memberships WHERE tenant_id = $1 AND role = 'owner'`,
			tenant.ID).Scan(&ownerCount); err != nil {
			t.Fatalf("iter %d: count owners: %v", i, err)
		}
		if ownerCount != 1 {
			t.Fatalf("iter %d: owner count after race = %d, want 1", i, ownerCount)
		}
	}
}

// seedInviteEncrypted is seedInvite plus the 2.2c encrypted-email columns,
// for tests that need a real round trip through storage.
func seedInviteEncrypted(t *testing.T, tenantSlug string, emailHash, emailEncrypted, emailNonce []byte, role, tokenHash string, expiresAt time.Time, invitedBy uuid.UUID) uuid.UUID {
	t.Helper()
	ctx := context.Background()
	tenant, err := newRepo().TenantBySlug(ctx, tenantSlug)
	if err != nil {
	...
	id := uuid.New()
	err = newRepo().CreateInvite(ctx, domain.Invite{
		ID: id, TenantID: tenant.ID, EmailLookupHash: emailHash,
		EmailEncrypted: emailEncrypted, EmailEncryptedNonce: emailNonce,
		Role: role, TokenHash: tokenHash, InvitedBy: invitedBy,
		ExpiresAt: expiresAt,
	})
	if err != nil {
...
func TestIntegration_ListInvites_ExcludesAcceptedAndRevoked_EmailRoundTrips(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	pendingID := seedInviteEncrypted(t, slug, emailHashFor(invitee1), []byte("cipher-a"), []byte("nonce-a"),
		domain.RoleEditor, "tok-pending", time.Now().Add(time.Hour), owner)
	acceptedID := seedInviteEncrypted(t, slug, emailHashFor(invitee2), []byte("cipher-b"), []byte("nonce-b"),
		domain.RoleViewer, "tok-accepted", time.Now().Add(time.Hour), owner)
	revokedID := seedInviteEncrypted(t, slug, emailHashFor(invitee3), []byte("cipher-c"), []byte("nonce-c"),
		domain.RoleViewer, "tok-revoked", time.Now().Add(time.Hour), owner)

	if _, err := repo.AcceptInvite(ctx, "tok-accepted", invitee2); err != nil {
		t.Fatalf("accept: %v", err)
	}
	if err := repo.RevokeInvite(ctx, tenant.ID, revokedID, owner); err != nil {
		t.Fatalf("revoke: %v", err)
	}

	invites, err := repo.ListInvites(ctx, tenant.ID)
	if err != nil {
		t.Fatalf("list invites: %v", err)
	}
	if len(invites) != 1 || invites[0].ID != pendingID {
		t.Fatalf("want only the pending invite (%s), got %+v", pendingID, invites)
	}
	if acceptedID == uuid.Nil || revokedID == uuid.Nil {
		t.Fatal("fixture ids should be set") // guards against a silently-broken fixture
	}
	if string(invites[0].EmailEncrypted) != "cipher-a" || string(invites[0].EmailEncryptedNonce) != "nonce-a" {
		t.Fatalf("encrypted email did not round-trip: %+v", invites[0])
	}
	// The raw token must never appear on this read path.
	if invites[0].TokenHash != "" {
		t.Fatalf("ListInvites must not populate TokenHash, got %q", invites[0].TokenHash)
	}
}

func TestIntegration_RevokeInvite_ThenAccept_ReturnsRevoked(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	tenant, _ := repo.TenantBySlug(ctx, slug)
	invitee := seedUser(t)
	seedInvite(t, slug, emailHashFor(invitee), domain.RoleEditor, "tok-rv", time.Now().Add(time.Hour), owner)

	var inviteID uuid.UUID
	if err := testPool.QueryRow(ctx, `SELECT id FROM tenant_invites WHERE token_hash = $1`, "tok-rv").Scan(&inviteID); err != nil {
		t.Fatalf("lookup invite id: %v", err)
	}

	if err := repo.RevokeInvite(ctx, tenant.ID, inviteID, owner); err != nil {
		t.Fatalf("revoke: %v", err)
	}
	// Idempotent: revoking again is a no-op success, not an error.
	if err := repo.RevokeInvite(ctx, tenant.ID, inviteID, owner); err != nil {
		t.Fatalf("second revoke should be a no-op success, got %v", err)
	}

	if _, err := repo.AcceptInvite(ctx, "tok-rv", invitee); err != domain.ErrInviteRevoked {
		t.Fatalf("accept after revoke err=%v, want ErrInviteRevoked", err)
	}
}

func TestIntegration_RevokeInvite_AlreadyAccepted(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	tenant, _ := repo.TenantBySlug(ctx, slug)
	invitee := seedUser(t)
	seedInvite(t, slug, emailHashFor(invitee), domain.RoleEditor, "tok-done", time.Now().Add(time.Hour), owner)

	var inviteID uuid.UUID
	if err := testPool.QueryRow(ctx, `SELECT id FROM tenant_invites WHERE token_hash = $1`, "tok-done").Scan(&inviteID); err != nil {
		t.Fatalf("lookup invite id: %v", err)
	}
	if _, err := repo.AcceptInvite(ctx, "tok-done", invitee); err != nil {
		t.Fatalf("accept: %v", err)
	}
	if err := repo.RevokeInvite(ctx, tenant.ID, inviteID, owner); err != domain.ErrInviteUsed {
		t.Fatalf("revoke-after-accept err=%v, want ErrInviteUsed", err)
	}
}

func TestIntegration_RevokeInvite_NotFound(t *testing.T) {
	requireIntegration(t)
	truncateTenants(t)
	ctx := context.Background()
	repo := newRepo()
	...
	if err := repo.RevokeInvite(ctx, tenant.ID, uuid.New(), owner); err != domain.ErrInviteNotFound {
