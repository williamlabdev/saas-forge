...
// (blind-index hash — no plaintext PII at rest — PLUS a reversibly encrypted
// copy, added in 2.2c/ADR-022 so a pending invite list can display the
// address it was sent to; the blind index alone cannot be decrypted).
// Granting Role in TenantID.
type Invite struct {
	ID                  uuid.UUID
	TenantID            uuid.UUID
	...
	// RevokedAt/RevokedBy record a deliberate owner/admin cancellation of a
	// still-pending invite (2.2c). Distinct from AcceptedAt/AcceptedBy — an
	// invite is consumed by exactly one of the two outcomes, never both.
	RevokedAt *time.Time
	RevokedBy *uuid.UUID
}
...
// Invite list statuses (2.2c GET /tenants/invites). "accepted" invites never
// appear in this list — the endpoint is scoped to PENDING invites only (its
// purpose is showing what is still outstanding), so the only two statuses a
// caller can observe here are these.
const (
	InviteStatusPending = "pending"
	InviteStatusExpired = "expired"
)

// Member is one row of a tenant's membership roster (2.2c GET
// /tenants/members). Email is resolved at the service layer from the user
// module's decrypted field — it is never stored on the membership itself.
type Member struct {
	UserID   uuid.UUID
	Email    string
	Role     string
...
// PendingInvite is one row of a tenant's outstanding-invite list (2.2c GET
// /tenants/invites). The raw token is NEVER part of this — or any — read
// path; only its hash is ever persisted (see Invite.TokenHash).
type PendingInvite struct {
	...
	ExpiresAt time.Time
	CreatedBy uuid.UUID
	Status    string // InviteStatusPending | InviteStatusExpired
}
