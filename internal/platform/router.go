...
// readyTimeout bounds the DB ping /ready performs. Short on purpose: this
// endpoint exists for an orchestrator's readiness probe, and a probe that can
// hang is worse than one that answers "not ready" promptly.
...
func NewRouter(
	...
	// dbPool backs /ready's DB ping. It is the same pool every other handler
	// uses (OpenVerifiedPool's one pool per process) — passed in rather than
	// opened here because both composition roots (cmd/server's wire graph and
	// BuildApp) already own one, and a second pool would be a second set of
	// connections to size and close.
	dbPool *pgxpool.Pool,
) http.Handler {
	r := chi.NewRouter()
	...
	// auth work (TKT-R7); no-op when unset. /health and /ready stay open for
	// probes — neither can carry the gateway secret.
	...
	// /ready is a SEPARATE probe from /health on purpose: /health answers
	// "the process is up" unconditionally (compose/orchestrator restart
	// policies key off it), while /ready answers "the process can currently
	// reach its database" — the thing worth taking a replica out of rotation
	// for. Conflating them would make a slow failover on Postgres look like a
	// crash-looping process.
	r.Get("/ready", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		if dbPool == nil {
			w.WriteHeader(http.StatusServiceUnavailable)
			_ = json.NewEncoder(w).Encode(map[string]string{"db": "not configured"})
			return
		}
