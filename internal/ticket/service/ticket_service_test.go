...
func (m *memRepo) Create(_ context.Context, x *domain.Ticket) error {
	m.items = append(m.items, x)
	return nil
}

func (m *memRepo) GetByID(_ context.Context, id uuid.UUID) (*domain.Ticket, error) {
...
func (m *memRepo) Update(_ context.Context, x *domain.Ticket) error {
...
func newTestService() TicketService {
	return NewTicketService(&memRepo{}, authz.NewAllowAllAuthorizer())
}
...
var _ repository.TicketRepository = (*memRepo)(nil)
...
// default, and an action-level rbac/opa rule ("may this subject read tickets?")
// is no help either — neither is given the record.
func TestService_CrossOwnerAccessDenied(t *testing.T) {
	owner, intruder := uuid.New(), uuid.New()
	repo := &memRepo{}
	svc := NewTicketService(repo, authz.NewAllowAllAuthorizer())
