...
func FromIR(raw []byte) (Artifact, []Dropped, error) {
	...
	for _, e := range doc.Entities {
		...
		for _, f := range e.Fields {
			// description SURVIVES the import as of migration 000046. It used
			// to be dropped here as inert documentation, and that was true
			// while nothing rendered it; ADR-007 Amendment 1 made it the line
			// the admin form composes a field's value rules into, so dropping
			// it now would silently delete the half a person wrote.
...
func NewArtifactFromTypes(a Artifact) Artifact {
	...
	for _, t := range a.Types {
		...
		for _, f := range t.Fields {
			// Every attribute is carried for the reason the permission lists
			// are, one comment up: the IR declares none of the component or
			// constraint ones today, so this changes nothing on the FromIR
			// path — and a re-ordering helper that quietly forgets half a
			// field is the loaded gun that note is about. description is the
			// case that proved it: it went missing here even after the
			// converter stopped dropping it.
			ct.Fields = append(ct.Fields, Field{
				Key: f.Key, Type: f.Type, Label: f.Label,
				Required: f.Required, Multiple: f.Multiple,
				EnumValues: f.EnumValues, RelationEntity: f.RelationEntity,
				ReadRoles: f.ReadRoles, WriteRoles: f.WriteRoles,
				ComponentName: f.Component, ZoneComponents: f.Components,
				Description:      f.Description,
