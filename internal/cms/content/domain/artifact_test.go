...
// A field without a description must serialise EXACTLY as it did before the
// attribute existed. That is the acceptance bar of the whole format (ADR-008
// Amendment 3): every schema already in someone's git tree re-exports
// byte-identical, or the first diff after an upgrade is noise on every field.
func TestArtifactOmitsAnAbsentDescription(t *testing.T) {
	plain, _ := MarshalArtifact(NewArtifact([]*ContentType{ct("post", Field{Key: "title", Type: FieldTypeString})}))
	if strings.Contains(string(plain), "description") {
		t.Fatalf("a field with no description wrote the key:\n%s", plain)
	}

	described, _ := MarshalArtifact(NewArtifact([]*ContentType{
		ct("post", Field{Key: "title", Type: FieldTypeString, Description: "Shown in the list."}),
	}))
	if !strings.Contains(string(described), `"description": "Shown in the list."`) {
		t.Fatalf("description did not reach the artifact:\n%s", described)
	}
}

// Rewording a description is graded like a label change and reported on its own
// line — additive, because no stored value can be made invalid by it.
func TestDiffReportsADescriptionChangeOnItsOwnLine(t *testing.T) {
	base := NewArtifact([]*ContentType{ct("post",
		Field{Key: "title", Type: FieldTypeString, Label: "Title", Description: "Shown in the list."},
	)})
	next := NewArtifact([]*ContentType{ct("post",
		Field{Key: "title", Type: FieldTypeString, Label: "Headline", Description: ""},
	)})

	changes := DiffSchemas(base, next)
	if len(changes) != 2 {
		t.Fatalf("expected the label and the description to be reported separately, got %+v", changes)
	}
	var sawDescription bool
	for _, c := range changes {
		if c.Grade != GradeAdditive {
			t.Fatalf("documentation graded %s: %+v", c.Grade, c)
		}
		if strings.Contains(c.Detail, "description") {
			sawDescription = true
		}
	}
	if !sawDescription {
		t.Fatalf("the emptied description was not reported: %+v", changes)
	}
}
