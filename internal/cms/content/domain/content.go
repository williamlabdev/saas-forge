...
const (
	...
	// FieldTypeComponent references a reusable component (ADR-020). The value
	// is one object (or, with Multiple, an array of objects) shaped by the
	// component's own sub-fields, stored INLINE in the entry payload.
	FieldTypeComponent = "component"
	// FieldTypeDynamicZone holds an ORDERED, HETEROGENEOUS list of component
	// items (ADR-020 Amendment 1). The field declares WHICH components it
	// accepts; each item carries ZoneDiscriminator naming the one it is, plus
	// that component's sub-field values. Unlike a `component` field — whose
	// shape is fixed by one component — a zone lets an editor compose a page
	// out of the allowed blocks in any order, which is the thing a repeatable
	// single-component field cannot express.
	FieldTypeDynamicZone = "dynamiczone"
)

// ZoneDiscriminator is the key each dynamic-zone item carries to name the
// component it is an instance of. It CANNOT collide with a sub-field key:
// identPattern requires an identifier to start with a letter, so no sub-field
// may begin with an underscore. That is why the discriminator lives INSIDE the
// item rather than wrapping it in {"__component":…,"data":{…}} — the flat form
// is what Strapi speaks, and the nesting would buy nothing but a second level
// for every error path to walk.
const ZoneDiscriminator = "__component"
...
//   - dynamiczone — permanently, and for the same reason as richtext: the value
//     is ALREADY a list. `multiple: true` on a zone would ask for a list of
//     lists, which no editor UI and no error path (details.item is one index)
//     can express. Absence from this list is what makes the service refuse it,
//     through the same errFieldMultipleUnsupported every other excluded type
//     gets — there is no second refusal to keep in sync.
//
...
func AllowedMultipleTypes() []string {
	return []string{FieldTypeString, FieldTypeEnum, FieldTypeRelation, FieldTypeNumber, FieldTypeComponent}
}
...
type Field struct {
	...
	// ComponentID / ComponentName / ComponentFields describe the component a
	// `component` field references (ADR-020). The id is the stored FK, the
	// name is what the API speaks, and the sub-fields are attached by the
	// repository on load so that validation, projection and the artifact can
	// work from the ContentType alone. All three are nil/empty on every other
	// type, and on a component's OWN sub-fields (nesting is refused).
	//
	// IMMUTABLE after creation, like Type, Multiple and RelationEntity: the
	// stored values are shaped by the referenced component.
	ComponentID     *uuid.UUID
	ComponentName   string
	ComponentFields []Field
	// ZoneComponents is the allowed-list of a `dynamiczone` field: the names of
	// the components an item may declare through ZoneDiscriminator. Non-empty
	// exactly when Type is dynamiczone, which a CHECK constraint enforces too
	// (migration 000045). ZoneFields carries each of those components'
	// sub-fields, attached by the repository on load, so validation, media
	// collection and the artifact can work from the ContentType alone — the
	// same job ComponentFields does one type up.
	//
	// The allowed-list is MUTABLE, unlike ComponentName: adding a name
	// invalidates no stored item, and removing one is refused while any entry
	// still holds an item of that component (CONTENT_ZONE_COMPONENT_IN_USE).
	// Names, not ids, are stored — a zone item names its component in the
	// payload, so the list and the items must speak the same alphabet, and a
	// component rename rewrites both in one transaction.
	ZoneComponents []string
	ZoneFields     map[string][]Field
	// Description is the schema author's own sentence about the field, shown to
	// whoever fills it in. Documentation, not a rule: nothing validates against
	// it and no stored value can contradict it, which is why it is freely
	// MUTABLE and carries no guard at all.
	//
	// It is not decoration either, and that is why it stopped being dropped at
	// the door (migration 000046): the admin form renders ONE helper line under
	// a control, and the value rules are composed into it. A field with both a
	// description and a constraint has that one line to say both things.
	Description string
	// FieldConstraints are the value-level rules (unique, format, pattern,
	// min/max). MUTABLE with guards: see field_constraints.go for why they are
	// attributes rather than types and which direction of change is the one-way
	// door.
	FieldConstraints
...
// ZoneAllows reports whether a dynamic zone accepts items of the named
// component. The list is short (an editor's block menu), so a scan beats a
// map the caller would have to build and keep in step with ZoneComponents.
func (f Field) ZoneAllows(name string) bool {
...
type Entry struct {
	...
	// SearchText is ExtractSearchText(schema, Payload) — the plain-text column
	// migration 000047 adds, computed at the SERVICE layer (the repository has
	// no schema access) and carried in here purely so the repository can bind
	// it as a SQL parameter next to Payload in the same INSERT/UPDATE. Every
	// service-layer write that sets Payload must also set this, or the search
	// column silently goes stale — see search.go's doc comment for the write
	// points this discipline applies to.
	SearchText string
	// PublishedSearchText mirrors SearchText for PublishedPayload. Unlike
	// SearchText it is USUALLY not Go-computed: SetEntryPublishState copies
	// search_text into published_search_text in the same SQL CASE expression
	// that copies payload into published_payload, since by the time a publish
	// runs, search_text is already correct on the row.
	PublishedSearchText string
