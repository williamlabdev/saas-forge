...
func TestDeliveryReads_CountedForDeliveryOnly(t *testing.T) {
	...
	if _, err := svc.GetEntry(admin, "order", e.ID, GetEntryInput{}); err != nil {
	...
	if _, err := svc.GetEntry(del, "order", e.ID, GetEntryInput{}); err != nil {
...
func TestDeliveryReads_NotCountedWhenNothingDelivered(t *testing.T) {
	...
	if _, err := svc.GetEntry(ctxDelivery("t1"), "order", draft.ID, GetEntryInput{}); err == nil {
