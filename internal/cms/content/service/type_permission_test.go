...
func TestTypePermission_EveryReadPathRefusesUnreadableCollection(t *testing.T) {
	...
	reads := map[string]func() error{
		...
		"get":  func() error { _, err := svc.GetEntry(editor, "ledger", e.ID, GetEntryInput{}); return err },
...
func TestTypePermission_DeliveryRoleOnTheListIsStillRefused(t *testing.T) {
	...
	_, err = svc.GetEntry(delivery, "ledger", e.ID, GetEntryInput{})
	assert.Equal(t, "CONTENT_TYPE_READ_FORBIDDEN", codeOf(t, err))
}
...
func TestConfinement_ForeignEntryIsNotFoundNotForbidden(t *testing.T) {
	...
	calls := map[string]func() error{
		"get": func() error { _, err := svc.GetEntry(editorAlice, "article", bobEntry, GetEntryInput{}); return err },
		"update": func() error {
...
func TestConfinement_RefusedWriteStoresNothing(t *testing.T) {
	...
	before, err := svc.GetEntry(admin, "article", bobEntry, GetEntryInput{})
	require.NoError(t, err)
	...
	after, err := svc.GetEntry(admin, "article", bobEntry, GetEntryInput{})
	require.NoError(t, err)
