...
func createRequestFingerprint(typeName string, in CreateLocalizedInput) ([]byte, error) {
	...
	// copy_from_source changes what the request MEANS — same Payload bytes,
	// same locale, same translation_of, but one asks for the body verbatim and
	// the other asks for it merged over the source. Two requests differing only
	// in this flag must not replay each other's result.
	copyFromSource := "0"
