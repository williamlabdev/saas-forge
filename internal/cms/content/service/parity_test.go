...
// content_type_fields_field_type_check constraint — as of 000045 (_v4, adding
// 'dynamiczone'), which REPLACED 000044's _v3, which replaced 000028's _v2.
// This test must always read the LATEST migration that touches the constraint;
// reading a superseded one would pin Go against a CHECK the database no longer
// enforces.
//
...
func TestFieldTypeSQLParity(t *testing.T) {
	...
	path := filepath.Join(filepath.Dir(file), "..", "migrations", "000045_content_dynamic_zone.up.sql")

	sql, err := os.ReadFile(path) //nolint:gosec // fixed test-local path
	require.NoError(t, err, "migration 000045 must exist — it defines the field_type contract")

	// CHECK (field_type IN ('string', 'text', ...))
	re := regexp.MustCompile(`(?is)CHECK\s*\(\s*field_type\s+IN\s*\(([^)]*)\)`)
	m := re.FindSubmatch(sql)
	require.NotNil(t, m, "content_type_fields_field_type_check not found in migration 000045")
