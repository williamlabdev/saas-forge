...
func TestPreview_ServesTheUnpublishedEntryItNames(t *testing.T) {
	...
	got, err := svc.GetEntry(ctxPreview("t1", e.ID), "order", e.ID, GetEntryInput{})
	if err != nil {
	...
	if _, err := svc.GetEntry(ctxDelivery("t1"), "order", e.ID, GetEntryInput{}); err != apperrors.ErrNotFound {
...
func TestPreview_TokenForOneEntryCannotReadAnother(t *testing.T) {
	...
	if _, err := svc.GetEntry(ctxPreview("t1", named.ID), "order", other.ID, GetEntryInput{}); err != apperrors.ErrNotFound {
