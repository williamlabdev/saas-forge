...
func TestFromIR_ReportsWhatItCannotCarry(t *testing.T) {
	...
	for _, want := range []string{"state_model.transitions", "ui_bindings"} {
		require.True(t, what[want], "loss not reported: %s (got %+v)", want, dropped)
	}
	// description is NOT on that list any more (ADR-008 Amendment 3): it used
	// to be dropped as inert documentation, and it now travels into the
	// artifact because the admin form renders it. Asserted both ways — that it
	// is no longer reported as a loss, and that the sentence actually arrived —
	// because "we stopped reporting it" and "we stopped dropping it" are two
	// different changes and only one of them is the right one.
	require.False(t, what["description"], "description reported as dropped, but it is carried now")
	require.Equal(t, "who booked", a.Types[0].Fields[0].Description)
