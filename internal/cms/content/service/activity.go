...
type activityRec struct {
	...
	title      string
	changed    []string
	// details is structured JSON ABOUT the action — never content values, the
	// same fence changed appear under. It exists because two verbs are
	// unreadable without it: entry.schedule.stale (which schedule, which
	// versions) and entry.restore (which revision, and what could not be carried
	// over). Both are rows about something that happened to an editor rather
	// than because of them, so the row has to carry enough to act on.
...
// withDetails attaches structured JSON about the action.
//
// KEYS AND FACTS, NEVER VALUES. changedKeys' comment states the fence and it
// applies here identically: a details object that carried the payload it
// restored would put content into a table that a viewer role can read without
// any field-level masking having run. Callers pass a domain helper
// (domain.RestoreDetails, domain.ScheduleStaleDetails) rather than an inline
// map, so the key spelling has one home and a test can pin it.
func (a *activityRec) withDetails(d json.RawMessage) *activityRec {
	if a == nil {
		return nil
	}
	a.details = d
...
func (a *activityRec) finish(ctx context.Context, err error) {
	...
	if err != nil {
		...
		a.changed = nil
		// Details goes with them, and for a sharper reason than symmetry: a
		// restore that was refused after computing its dropped_keys would leave a
		// row stating which keys a restore discarded, when no restore happened
		// and nothing was discarded. The error code is the whole of what a
		// refused line has to say.
		a.details = nil
	}
...
type ActivityDTO struct {
	...
	// Via and ViaScheduleID say HOW the action was carried out: "" for the
	// ordinary case, "schedule" when a worker performed it on the authority of
	// an intent this actor filed earlier (ADR-017).
	//
	// No `omitempty`, for the reason this type's header gives — and here the
	// reason bites harder than usual. A console that omits `via` renders a
	// scheduled 03:00 publish as a person publishing at 03:00, which is not a
	// missing detail but a false statement about who was at the keyboard.
	Via           string     `json:"via"`
	ViaScheduleID *uuid.UUID `json:"via_schedule_id"`

	// Details is structured JSON about the action and never content values.
	// null for every action but entry.schedule.stale today, where it carries
	// schedule_id, pinned_version and entry_version — the three facts an editor
	// needs to decide whether to re-file the release (ADR-017 §2).
