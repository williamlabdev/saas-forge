...
func TestSchemaMutation_FieldTypeCheckConstraint(t *testing.T) {
	...
	// A component field must name the component it embeds (000044's
	// content_type_fields_component_ref_check), so the loop below hands it one;
	// every other type must NOT carry one, which the same CHECK enforces.
	componentID := uuid.New()
	if _, err := pool.Exec(ctx,
		`INSERT INTO content_components (id, tenant_id, name, label) VALUES ($1,'t','seo','')`, componentID); err != nil {
		t.Fatal(err)
	}
	// A dynamic zone is the mirror image (000045's
	// content_type_fields_zone_components_check): it must carry a non-empty
	// allowed list, and no other type may carry one.
	insert := func(fieldType string) error {
		var ref *uuid.UUID
		if fieldType == domain.FieldTypeComponent {
			ref = &componentID
		}
		var zone []string
		...
		_, err := pool.Exec(ctx,
			`INSERT INTO content_type_fields (id, content_type_id, key, field_type, ref_component_id, zone_components)
			 VALUES ($1,$2,$3,$4,$5,$6)`,
			uuid.New(), typeID, "k_"+fieldType, fieldType, ref, zone)
		return err
	}
	...
	// The zone constraint is an IFF, and both halves matter. A zone with an
	// empty list would leave the validator with nothing to allow, so every item
	// of the field would be refused; a list on any other type is a value
	// nothing reads, which is how a rename comes to miss a copy.
	if _, err := pool.Exec(ctx,
		`INSERT INTO content_type_fields (id, content_type_id, key, field_type, zone_components) VALUES ($1,$2,'z_empty','dynamiczone','{}')`,
		uuid.New(), typeID); err == nil {
		t.Fatal("expected content_type_fields_zone_components_check to reject an empty allowed list")
	}
	if _, err := pool.Exec(ctx,
		`INSERT INTO content_type_fields (id, content_type_id, key, field_type, zone_components) VALUES ($1,$2,'z_string','string','{seo}')`,
		uuid.New(), typeID); err == nil {
		t.Fatal("expected content_type_fields_zone_components_check to reject zone_components on a non-zone field")
	}
	// And nesting is refused in the DATABASE too, not only in Go: a sub-field
	// row (content_type_id NULL, component_id set) may not be a dynamic zone —
	// 000045's content_type_fields_zone_nesting_check, the twin of 000044's
	// component one.
	if _, err := pool.Exec(ctx,
		`INSERT INTO content_type_fields (id, component_id, key, field_type, zone_components) VALUES ($1,$2,'z','dynamiczone','{seo}')`,
		uuid.New(), componentID); err == nil {
		t.Fatal("expected a dynamiczone component sub-field to be refused by the database")
	}
}
