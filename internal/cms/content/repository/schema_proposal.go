...
type SchemaProposal struct {
	...
	DecidedAt       *time.Time
	DecidedBy       *uuid.UUID
	// AppliedSteps is the JSONB array of indices, into the STORED plan's steps
	// array, that DecideSchemaProposal actually ran — written in the SAME UPDATE
	// that flips status to approved, so there is no window where the row says
	// approved without yet saying what ran (缺口計畫 5.1).
	//
	// NULL for pending, rejected, and expired rows, and for every row approved
	// before this column existed. It is never an empty array on an approved row
	// written by this path: a full approval writes every executed index, so the
	// only way to see NULL on an approved row is that it predates the column.
	AppliedSteps []byte
}
...
const proposalColumns = `id, tenant_id, artifact, prune, plan, status,
	...
	created_at, expires_at, decided_at, decided_by, applied_steps`
...
func proposalDests(p *SchemaProposal) []any {
	return []any{&p.ID, &p.TenantID, &p.Artifact, &p.Prune, &p.Plan, &p.Status,
		...
		&p.CreatedAt, &p.ExpiresAt, &p.DecidedAt, &p.DecidedBy, &p.AppliedSteps}
}
...
//
// appliedSteps is written in this SAME UPDATE (缺口計畫 5.1), not a second
// statement after it: a status flip that landed without its applied-steps row,
// or the reverse, is exactly the "recorded but did not happen" split ADR-013's
// one-transaction rule exists to rule out one level up. It is JSON-encoded
// bytes rather than a typed slice because the repository does not know the
// service's step-index type any more than it knows PlanResult — see
// SchemaProposal's own comment on Artifact and Plan. nil means NULL: a
// rejection carries no applied steps, and neither does an approval this
// package predates.
func (r *PostgresContentRepository) DecideSchemaProposal(ctx context.Context, tenantID string, id uuid.UUID,
	...
	return r.withTenant(ctx, tenantID, func(q querier) error {
		tag, err := q.Exec(ctx, `
			...
			SET status = $1, decided_at = $2, decided_by = $3, applied_steps = $6
			WHERE tenant_id = $4 AND id = $5 AND status = 'pending' AND expires_at > $2`,
			status, now, decidedBy, tenantID, id, appliedSteps)
		if err != nil {
