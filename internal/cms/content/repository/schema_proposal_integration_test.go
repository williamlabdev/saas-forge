...
func TestSchemaProposals(t *testing.T) {
	...
	t.Run("deciding is a one-way door, enforced by the WHERE clause", func(t *testing.T) {
		p := file(t, "t", now.Add(time.Hour))
		decider := uuid.New()
		if err := repo.DecideSchemaProposal(ctx, "t", p.ID, ProposalApproved, decider, now, nil); err != nil {
		...
		err := repo.DecideSchemaProposal(ctx, "t", p.ID, ProposalRejected, uuid.New(), now, nil)
	...
	t.Run("applied_steps round-trips and stays NULL until a decision writes it", func(t *testing.T) {
		p := file(t, "t", now.Add(time.Hour))
		before, err := repo.GetSchemaProposal(ctx, "t", p.ID)
		if err != nil {
			t.Fatal(err)
		}
		if before.AppliedSteps != nil {
			t.Fatalf("applied_steps=%s — a pending row must not carry one", before.AppliedSteps)
		}
		if err := repo.DecideSchemaProposal(ctx, "t", p.ID, ProposalApproved, uuid.New(), now,
			[]byte(`[0,2,3]`)); err != nil {
			t.Fatalf("decide with applied_steps: %v", err)
		}
		got, err := repo.GetSchemaProposal(ctx, "t", p.ID)
		if err != nil {
			t.Fatal(err)
		...
		var applied []int
		if err := json.Unmarshal(got.AppliedSteps, &applied); err != nil {
			t.Fatalf("applied_steps did not round-trip as JSON: %v (%s)", err, got.AppliedSteps)
		}
		if len(applied) != 3 || applied[0] != 0 || applied[1] != 2 || applied[2] != 3 {
			t.Fatalf("applied_steps=%v — the SAME UPDATE that flipped status did not carry the indices it was given", applied)
		}

		// A second row, rejected: DecideSchemaProposal's reject path passes nil,
		// same as the service does, and NULL is what a reader must see back —
		// not [] and not the zeroed slice a lazier column default would give.
		p2 := file(t, "t", now.Add(time.Hour))
		if err := repo.DecideSchemaProposal(ctx, "t", p2.ID, ProposalRejected, uuid.New(), now, nil); err != nil {
			t.Fatalf("decide rejected: %v", err)
		}
		got2, err := repo.GetSchemaProposal(ctx, "t", p2.ID)
		if err != nil {
			t.Fatal(err)
		}
		if got2.AppliedSteps != nil {
			t.Fatalf("applied_steps=%s — a rejected row applied nothing and must stay NULL", got2.AppliedSteps)
		}
	})

	t.Run("an expired proposal cannot be decided at all", func(t *testing.T) {
		p := file(t, "t", now.Add(-time.Minute))
		err := repo.DecideSchemaProposal(ctx, "t", p.ID, ProposalApproved, uuid.New(), now, nil)
		if !errors.Is(err, ErrProposalNotPending) {
	...
	t.Run("another tenant's proposal is not reachable", func(t *testing.T) {
		...
		if err := repo.DecideSchemaProposal(ctx, "t", theirs.ID, ProposalApproved, uuid.New(), now, nil); err == nil {
