...
func (r *PostgresContentRepository) RecordActivity(ctx context.Context, a *domain.Activity) error {
	return r.withTenant(ctx, a.TenantID, func(q querier) error {
		_, err := q.Exec(ctx, `
			...
			a.Outcome, a.ErrorCode, orEmptyKeys(a.ChangedKeys), a.Via, a.ViaScheduleID,
			orNilJSON(a.Details),
		)
		if err != nil {
	...
	err := r.withTenant(ctx, f.TenantID, func(q querier) error {
		...
		for rows.Next() {
			...
			if err := rows.Scan(
				...
				&a.Action, &a.TargetType, &a.TargetEntryID, &a.TargetTitle,
				&a.Outcome, &a.ErrorCode, &a.ChangedKeys, &a.Via, &a.ViaScheduleID, &a.Details,
			); err != nil {
				return err
			}
			out = append(out, &a)
...
// orNilJSON keeps an empty Details out of the column as NULL rather than as the
// four bytes "null". A JSONB holding the json null is not absent — it is a row
// that says "the detail is null", and `details IS NULL` would stop finding it.
func orNilJSON(b json.RawMessage) any {
	if len(b) == 0 {
		return nil
	}
	return []byte(b)
