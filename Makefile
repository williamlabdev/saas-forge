.PHONY: test test-coverage test-coverage-html test-integration test-e2e sqlc wire mocks build migrate-status migrate-up up down opa-test mcp-mock govulncheck lint new-domain new-domain-smoke fmt prod-config prod-up prod-down backup restore-drill sdk-ts-gen sdk-ts-check
...
# Production stack (deploy/docker-compose.prod.yml). See docs/DEPLOYMENT.md.
# `.env.production` (untracked; copy from .env.production.example) is
# required for all four targets.
#
# IMAGE_TAG has no default in the compose file (a bare `latest` drifts under
# you); these targets default it to the current commit's short sha when it
# isn't already set in the environment or .env.production, so a plain
# `make prod-up` still works for local/dev use.
IMAGE_TAG ?= $(shell git rev-parse --short HEAD)

prod-config:
	IMAGE_TAG=$(IMAGE_TAG) docker compose -f deploy/docker-compose.prod.yml --env-file .env.production config

prod-up:
	IMAGE_TAG=$(IMAGE_TAG) docker compose -f deploy/docker-compose.prod.yml --env-file .env.production up -d --build

prod-down:
	IMAGE_TAG=$(IMAGE_TAG) docker compose -f deploy/docker-compose.prod.yml --env-file .env.production down
...
# sdk/delivery-ts is generated FROM apps/delivery/openapi/delivery.yaml, not
# maintained separately — regenerate and commit the result whenever that spec
# changes. apps/delivery/internal/handler/openapi_test.go is what tells you a
# change is needed: it fails the Go build the day the spec drifts from the
# routes/params/error codes the edge actually serves.
sdk-ts-gen:
	cd sdk/delivery-ts && pnpm install && pnpm run gen

# typecheck + test the TS client against the COMMITTED generated types (no
# `pnpm gen` here) — this is the check that a consumer who only ran
# `pnpm install`, with no Go toolchain, would see.
sdk-ts-check:
	cd sdk/delivery-ts && pnpm install && pnpm run typecheck && pnpm run test
